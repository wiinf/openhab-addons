 ...
 */
package org.openhab.binding.modbus.deyeinverter.internal.handler;

import static org.openhab.binding.modbus.deyeinverter.internal.ModbusDeyeInverterBindingConstants.*;
...
import org.openhab.binding.modbus.deyeinverter.internal.config.DeyeInverterConfiguration;
import org.openhab.binding.modbus.deyeinverter.internal.dto.DeyeInverterRegisters;
...
/**
 * The {@link Deye3PhaseInverterHandler} is responsible for reading the Modbus values of the
 * Deye 3Phase inverter.
 *
 * @author Holger Friedrich - Initial contribution
 * @author Alexander Teterra - carry over from Fox ESS Binding
 */
@NonNullByDefault
public class DeyeInverterHandler extends BaseModbusThingHandler {
    ...
    private static final class ModbusRequest {
        ...
        private final Deque<DeyeInverterRegisters> registers;
        private final ModbusReadRequestBlueprint blueprint;

        public ModbusRequest(Deque<DeyeInverterRegisters> registers, int slaveId, int tries) {
            this.registers = registers;
            this.blueprint = initReadRequest(registers, slaveId, tries);
        }

        private ModbusReadRequestBlueprint initReadRequest(Deque<DeyeInverterRegisters> registers, int slaveId,
                int tries) {
            ...
            int registerReadSize = Objects.requireNonNull(registers.getLast()).getRegisterCount();
            if (registerReadSize < 1) {
                registerReadSize = 1;
            }
            int length = lastRegister - firstRegister + registerReadSize;
    ...
    private final Logger logger = LoggerFactory.getLogger(DeyeInverterHandler.class);
    ...
    public DeyeInverterHandler(Thing thing, final TranslationProvider translationProvider,
    ...
    private List<ModbusRequest> buildRequests(int tries) {
        ...
        Deque<DeyeInverterRegisters> currentRequest = new ArrayDeque<>();
        int currentRequestFirstRegister = 0;

        for (DeyeInverterRegisters channel : DeyeInverterRegisters.values()) {
    ...
    public void modbusInitialize() {
        final DeyeInverterConfiguration config = getConfigAs(DeyeInverterConfiguration.class);
        ...
        if (d == null) {
            ...
            switch (channel.getChannelName().substring(HIDDEN_CHANNEL_PREFIX.length())) {
                ...
                case CHANNEL_GRID_FREQUENCY:
                    ...
                    updateState(new ChannelUID(thing.getUID(), "fi-" + channel.getChannelGroup(),
                            "di-" + CHANNEL_STATUS_ON_GRID), state);
                    break;
                ...
                case CHANNEL_STATUS1:
                    ...
                    updateState(new ChannelUID(thing.getUID(), "fi-" + channel.getChannelGroup(),
                            "di-" + CHANNEL_STATUS_STANDBY), stateUpdate);
                    stateUpdate = statusOperation ? OpenClosedType.OPEN : OpenClosedType.CLOSED;
                    logger.debug("{} {} -> {}", CHANNEL_STATUS_OPERATION.toUpperCase(), statusOperation, d);
                    updateState(new ChannelUID(thing.getUID(), "fi-" + channel.getChannelGroup(),
                            "di-" + CHANNEL_STATUS_OPERATION), stateUpdate);
                ...
                case CHANNEL_EPS_OUTPUT:
                    ...
                    updateState(new ChannelUID(thing.getUID(), "di-" + channel.getChannelGroup(),
                            "di-" + CHANNEL_EPS_OUTPUT), epsState);
                    break;
                default:
    ...
    private void readSuccessful(ModbusRequest request, AsyncModbusReadResult result) {
        ...
        result.getRegisters().ifPresent(registers -> {
            ...
            for (DeyeInverterRegisters channel : request.registers) {
                int index = channel.getRegisterNumber() - firstRegister;
                if (channel.getType() == ValueType.BIT) {
                    index = (index * 16) + channel.getBitIndex();
                }
    ...
    private ChannelUID createChannelUid(DeyeInverterRegisters register) {
        return new ChannelUID(thing.getUID(), "di-" + register.getChannelGroup(), "di-" + register.getChannelName());
    }
