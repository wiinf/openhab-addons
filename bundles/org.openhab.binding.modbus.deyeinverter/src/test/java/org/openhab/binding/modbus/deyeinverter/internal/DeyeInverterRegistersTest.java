 ...
 * @author Alexander Teterra - Carry over from FoxESS
 */
@NonNullByDefault
class DeyeInverterRegistersTest {
    @Test
    public void testCreatePercentTypeState() {
        // DeyeInverterRegisters batteryLevelRegister = DeyeInverterRegisters.BATTERY_LEVEL;
        //
        // ModbusRegisterArray registers = new ModbusRegisterArray(100);
        // Optional<DecimalType> value = ModbusBitUtilities.extractStateFromRegisters(registers, 0,
    ...
    public void testCreateQuantityTypeState() {
        DeyeInverterRegisters mpptVoltage = DeyeInverterRegisters.MPPT1_VOLTAGE;

        ModbusRegisterArray registers = new ModbusRegisterArray(1234);
        Optional<DecimalType> value = ModbusBitUtilities.extractStateFromRegisters(registers, 0, mpptVoltage.getType());
        assertTrue(value.isPresent());
        DecimalType decimalTypeValue = value.get();
        // Value is not scaled yet
